# Claude Code Project Instructions — pkg-config-kotlin

## Project Overview

This is **pkg-config-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`pkg-config`](https://crates.io/crates/pkg-config).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/pkg-config/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/pkg-config>
