...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `PkgConfig.swiftmodule/`
...
//      `import PkgConfig` above would fail at compile time.
//
//   2. The static archive `libPkgConfig.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "PkgConfig" }` and
//      `flattenPackage = "io.github.kotlinmania.pkgconfig"` configuration in
...
//      via the `PkgConfigLibrary` product.
//
...
final class PkgConfigExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "PkgConfig swift module imported cleanly")
    }
}
